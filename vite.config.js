...
const antiFoucRevealScript = [
  ...
  'var isNav=false;try{isNav=sessionStorage.getItem("navigating")==="true";sessionStorage.removeItem("navigating");}catch(e){}',
  'var currentWidth=isNav?70:0;var targetWidth=isNav?75:15;var animId=null;',
  'function update(){',
  ...
  'animId=requestAnimationFrame(update);',
  'waitForStyles().then(function(){targetWidth=Math.max(targetWidth,isNav?80:45);});',
  'waitForFonts().then(function(){targetWidth=Math.max(targetWidth,isNav?85:65);});',
  'waitForFlagOrEvent("__navReady","codedge:nav-ready",1200).then(function(){targetWidth=Math.max(targetWidth,isNav?90:80);});',
  'waitForFlagOrEvent("__footerReady","codedge:footer-ready",1200).then(function(){targetWidth=Math.max(targetWidth,isNav?95:90);});',
  ...
  'if((href.indexOf("http")===0&&href.indexOf(window.location.origin)!==0)||/^(mailto:|tel:|javascript:|#)/i.test(href)||link.getAttribute("target")==="_blank"||link.hasAttribute("download"))return;',
  'var targetUrl;try{targetUrl=new URL(href,window.location.origin);}catch(err){return;}',
  'if(targetUrl.pathname===window.location.pathname&&targetUrl.hash)return;',
  'e.preventDefault();',
  'try{sessionStorage.setItem("navigating","true");}catch(err){}',
  'var cBar=document.querySelector(".loading-bar-top");',
  'if(cBar){',
  'var cW=parseFloat(cBar.style.width)||0;var tW=75;',
  'function step(){cW+=(tW-cW)*0.15;cBar.style.width=cW+"%";if(cW<74.5)requestAnimationFrame(step);}',
  'requestAnimationFrame(step);',
  '}',
  'var body=document.body;if(body){',
  'body.setAttribute("data-css-ready","pending");',
  'var children=body.children;',
  'for(var i=0;i<children.length;i++){',
  'var child=children[i];',
  'if(child!==cBar&&child.tagName!=="SCRIPT"){',
