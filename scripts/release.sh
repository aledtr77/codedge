...
# moved fifteen commits past it, so anyone downloading from the Releases page
# got a source snapshot without the accessibility suite, without the English
# source tree, and with a debug line still logging to the console of the live
# site. Nothing was broken and nothing was red. The drift is invisible by
...
# checking at all.
#
# The slow half of CI (test:smoke, test:a11y) is not repeated here: both need a
