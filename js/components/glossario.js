document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleSearch() {
        ...
        Array.from(details).forEach(detail => {
            ...
            if (searchTerm === '') {
                detail.open = false;
            } else if (detail.hasAttribute('data-priority') && summaryText.toLowerCase().startsWith(searchTerm)) {
                detail.open = true;
        ...
        if (matchToScroll) {
            setTimeout(() => {
                ...
                const y = matchToScroll.getBoundingClientRect().top + window.pageYOffset + yOffset;
    ...
    if (menuIcon && navMenu) {
        ...
        document.addEventListener('click', function (event) {
            const isClickInside = navMenu.contains(event.target) || menuIcon.contains(event.target);
